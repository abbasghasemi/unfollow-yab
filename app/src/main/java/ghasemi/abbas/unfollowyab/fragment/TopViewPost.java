...
public class TopViewPost extends BaseFragment {
    ...
    protected View.OnClickListener buttonRightClick() {
        return view -> createDialog("پست های که بیشترین بازدید را داشته اند.");
    }
    ...
    public void onCreateFragment(@Nullable Bundle savedInstanceState) {
        ...
        setPageTitle("پر بازدید ترین پست ها");
    ...
    private void load() {
        bundles = SQL.getSql().getViewPosts();
        if (bundles.isEmpty()) {
            notFound();
        } else {
            setPageTitle(bundles.size() + " پست با بیشترین بازدید");
